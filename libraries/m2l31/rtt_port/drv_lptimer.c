...
{
#if defined(BSP_USING_LPTMR0)
    { .name = "lptmr0", .base  = LPTMR0, .irqn = LPTMR0_IRQn, .rstidx = LPTMR0_RST, .modid = LPTMR0_MODULE  },
#endif
#if defined(BSP_USING_LPTMR1)
    { .name = "lptmr1", .base  = LPTMR1, .irqn = LPTMR1_IRQn, .rstidx = LPTMR1_RST, .modid = LPTMR1_MODULE  },
#endif
};
