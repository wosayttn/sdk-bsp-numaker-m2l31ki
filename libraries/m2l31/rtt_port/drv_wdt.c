...
static uint32_t wdt_get_module_clock(void)
{
    return (CLK_GetModuleClockSource(WDT_MODULE) << LPSCC_CLKSEL0_WDTSEL_Pos);
}
...
static uint32_t wdt_get_working_hz(void)
{
    ...
    switch (clk)
    {
    case LPSCC_CLKSEL0_WDTSEL_LIRC:
        hz = __LIRC;
        break;

    case LPSCC_CLKSEL0_WDTSEL_LXT:
        hz = __LXT;
        break;

    case LPSCC_CLKSEL0_WDTSEL_HCLK1_DIV2048:
