...
static int nu_pdma_non_transfer_count_get(int32_t i32ModChnID);
...
static struct nu_pdma nu_pdma_arr[] =
{
    ...
    [1] =
    {
        .m_module = {
            .name = "lppdma",
            .m_pvBase = (void *)LPPDMA0,
            .u32RstId = LPPDMA0_RST,
            .eIRQn = LPPDMA0_IRQn
        }
    }
};
...
static void nu_pdma_init(void)
{
    ...
    {
        PDMA_T *psPDMA = (PDMA_T *)nu_pdma_arr[i].m_module.m_pvBase;
        nu_pdma_chn_mask_arr[i] = ~(NU_PDMA_CH_Msk);

        if (psPDMA == (PDMA_T *)LPPDMA0)
        {
            nu_pdma_arr[i].m_psSGTbl = rt_memheap_alloc(nu_get_lpmemheap(), sizeof(DSCT_T) * NU_PDMA_SGTBL_POOL_SIZE);
        }
        else
        {
        ...
        psPDMA->SCATBA = (uint32_t)nu_pdma_arr[i].m_psSGTbl;
        rt_kprintf("Set %s SCATBA address to 0x%08x.\n", nu_pdma_arr[i].m_module.name, psPDMA->SCATBA);
    }
...
static rt_err_t nu_pdma_timeout_set(int i32ModChnID, int i32Timeout_us)
{
    ...
    if (i32Timeout_us && (u32ModChannId < 2))
    {
        ...
        pdma->TOUTPSC = (pdma->TOUTPSC & ~(0x7ul << (PDMA_TOUTPSC_TOUTPSC0_Pos * u32ModChannId))) | (u32Divider << (PDMA_TOUTPSC_TOUTPSC0_Pos * u32ModChannId));
...
static rt_err_t _nu_pdma_transfer_chain(int i32ModChnID, uint32_t u32DataWidth, uint32_t u32AddrSrc, uint32_t u32AddrDst, uint32_t u32TransferCnt, uint32_t u32IdleTimeout_us)
{
    ...
    if (psPdmaChann->m_u32WantedSGTblNum != (u32TransferCnt / NU_PDMA_MAX_TXCNT + 1))
    {
        ...
        {
            psPdmaChann->m_ppsSgtbl = (nu_pdma_desc_t *)rt_memheap_alloc(nu_get_lpmemheap(), sizeof(nu_pdma_desc_t) * psPdmaChann->m_u32WantedSGTblNum);
        }
        else
        {
...
void LPPDMA0_IRQHandler(void)
{
    ...
    pdma_isr((PDMA_T *)LPPDMA0, LPPDMA_IDX);
