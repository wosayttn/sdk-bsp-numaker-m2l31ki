 ...
 *
 */
#define RMC_GET_ISP_BANK_SELECTION()     ((uint32_t)((RMC->ISPSTS & RMC_ISPSTS_FBS_Msk) >> RMC_ISPSTS_FBS_Pos))
...
void     RMC_Close(void);
int32_t  RMC_ConfigXOM(uint32_t xom_num, uint32_t xom_base, uint8_t xom_page);
int32_t  RMC_Erase(uint32_t u32PageAddr);
int32_t  RMC_EraseXOM(uint32_t xom_num);
int32_t  RMC_GetXOMState(uint32_t xom_num);
